...
var Image = (function (){
  ...
  Image.prototype.build = async function (context, src, uid, { onFinished, onProgress}){ 
    ...
    let onFinish = function (err, res) {
      ...
      if (typeof onFinished == 'function') onFinished(err, res);
    };
    if (stream) {
      this.id = uid;
      docker.modem.followProgress(stream, onFinish.bind(this), onProgress);
    }
