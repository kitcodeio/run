...
module.exports = config => {
  return {
    async removeImage(request, reply) {
      ...
      let image = await docker.getImage(image_id).inspect().catch(err => console.log(err));
    ...
    async start(request, reply) {
      let { image_id, subdomain, test } = request.payload;
      ...
      if (!test) shelljs.exec("sudo bash " + __dirname + "/virtualhost.sh " + subdomain + " " + data.NetworkSettings.IPAddress, {
        silent: true
      });
      return reply(data);
    },
    async stop(request, reply) {
      ...
      try {
        ...
        if (!test) shelljs.exec("sudo bash " + __dirname + "/delete-virtualhost.sh " + subdomain, {
          silent: true
        });
        return reply({
          statusCode: 200
